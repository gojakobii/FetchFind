...
function Pagination({page, totalPages, size, showSize, totalResults, setPage}) {
    ...
    <div className="flex mt-6 items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
        ...
        <div className="flex flex-1 justify-between items-center sm:hidden">
          ...
          <div>
            <p className="text-sm font-lexend text-gray-700">
              Showing <span className="font-medium text-[#800f74]">{(page-1)*size + 1}</span> to <span className="font-medium text-[#800f74]">{(page-1)*size + showSize}</span> of{' '}
              <span className="font-medium text-[#800f74]">{totalResults}</span> results
            </p>
          </div>
          {page < totalPages && (
            <a
      ...
      {totalPages > 0 && (
        <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
          ...
          <nav className="isolate flex gap-2 -space-x-px rounded-md" aria-label="Pagination">
            {page > 1 && (
              <a
              href="#"
              onClick={handlePrevPage}
            ...
            >
              <ChevronLeftIcon className="h-[21px] w-[21px]" aria-hidden="true" />
              Previous
            </a>
            )}
            {page < totalPages && (
            ...
            >
              Next
              <ChevronRightIcon className="h-[21px] w-[21px]" aria-hidden="true" />
            </a>
            )}
          </nav>
        </div>
